...
class Env_rouge_ID(gym.Env):
    ...
    def step(self,action):
        ...
        print(next_token_id)
        self.outputs = self.language_model(next_token_id, past_key_values=self.past_kv, use_cache=True)
        self.past_kv = self.outputs.past_key_values
        self.curr_ids = torch.cat([self.curr_ids, next_token_id], dim=-1) #?
        ...
        self.length += 1
        terminate = self.length == self.horizon or next_token_id == self.eos_id or self.repeat()
        reward = 0
        if terminate:
            self.summary = self.tokenizer.decode(self.input_ids[0],skip_special_tokens=True)
            reward = self.rouge_reward(self.summary,self.batch['summary']) if terminate else 0
        print(reward)
        return self.ids_to_obs(self.curr_ids), reward, terminate, False, {}

    def rouge_reward(self,gen,ref):
    ...
    def reset(self, *, seed=None, options=None):
        ...
        try:
            ...
            self.batch = next(self.batch_it)
        self.input_ids = self.tokenizer(self.batch['dialogue'], return_tensors="pt").input_ids
